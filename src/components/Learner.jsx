...
class Learner extends React.PureComponent {
    ...
    maxIndex = () => {
        return Math.min(characters.length, (this.state.level * charactersPerLevel))
    }

    newWordIndexes = () => {
        const newWordIndexes = []
        for (let i = 0; i < this.maxIndex(); i++) {
            if (this.state.correctStreaks[i] === 0) {
    ...
    setNewWord = () => {
        if (showNewChance >= Math.random() || this.state.guaranteeNew) {
            const newWordIndexes = this.newWordIndexes()
            const newIndex = newWordIndexes[Math.floor(Math.random() * newWordIndexes.length)]
    ...
    calculateKnownCharacters = () => {
        return this.state.correctStreaks.reduce((total, current) => total + (current >= 1 ? 1 : 0), 0)
    }

    save = () => {
        localStorage.setItem('level', this.state.level)
        localStorage.setItem('correctStreaks', JSON.stringify(this.state.correctStreaks))
    ...
    loadSave = () => {
        let promise = new Promise((resolve, reject) => {
            if (localStorage.getItem('hasSave')) {
                const correctStreaks = JSON.parse(localStorage.getItem('correctStreaks'))
                ...
                this.setState({
                    level: 1,
                    correctStreaks: Array(characters.length).fill(0)
                }, () => {
                    resolve()
                })
            }
    ...
    handleChange = (e) => {
        if (e.target.value.split(' ').join('') === noTones[this.state.activeIndex].split(' ').join('')) {
            let updatedCorrectStreaks = []
            let increaseLevel = false
            if (!this.state.showAnswer) {
                updatedCorrectStreaks = this.updateListItem(this.state.activeIndex, this.state.correctStreaks[this.state.activeIndex] + 1)
                // This is a new learned characters
                if (updatedCorrectStreaks[this.state.activeIndex] === 1) {
                    if (this.calculateKnownCharacters() + 1 >= this.state.level * charactersPerLevel) {
                        increaseLevel = true
                    }
                }
            } else {
                updatedCorrectStreaks = this.updateListItem(this.state.activeIndex, 0)
            }
            this.setState({correctStreaks: updatedCorrectStreaks}, () => {
                this.save()
            })
            this.setState({input: '', showAnswer: false})
            if (increaseLevel) {
                this.setState({level: this.state.level + 1, guaranteeNew: true}, () => {
                    this.save()
                    this.setNewWord()
                })
            } else {
    ...
    skip = () => {
        const updatedCorrectStreaks = this.updateListItem(this.state.activeIndex, 0)

        this.setState({
            input: '',
            showAnswer: false,
            correctStreaks: updatedCorrectStreaks
        }, () => {
            this.save()
        })
        this.setNewWord()
    ...
    updateListItem = (index, newValue) => {
        return this.state.correctStreaks.map((streak, i) => index === i ? newValue : streak)
    }
            ...
            <div style={styles.container}>
                <div style={styles.infoContainer}>
                    <div style={styles.infoSection}>
                        {`Unknown Characters: ${this.state.level * charactersPerLevel - this.calculateKnownCharacters()}`}
                    </div>
                    <div style={styles.infoSection}>
                        {`Known Characters: ${this.calculateKnownCharacters()}`}
                    </div>
                    <div style={styles.infoSection}>
                        {`Level: ${this.state.level}`}
                    </div>
                </div>
                <div style={styles.learnerContainer}>
